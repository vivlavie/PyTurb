...
# for t in ['T05']:    
    ...
    if (os.path.exists(fn) == False):
        ...
        for z in Zs:
            ...
            zi += 1
        print "k: {:10s}".format(Vd[Ts.index(t)]), " ".join(map("{:8.2f}".format,Ks))
        # print "sigma: {:10s}".format(Vd[Ts.index(t)]), " ".join(map("{:8.2f}".format,np.sqrt(Ks/c_iso)))
        # print "sigma: {:10s}".format(Vd[Ts.index(t)]), " ".join(map("{:8.2f}".format,s))
        TKE[ti] = max(Ks)
        # sigma[ti] = max(s)
